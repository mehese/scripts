...
from astools.analysis import rdf, rdf2, rdf_triclinic
...
a = ReadStruct('../crystal_files/INPUT_mHfO2_nosymm')
x, y = rdf(a, 200, dist=8.)
x_ = np.linspace(np.min(x), np.max(x), 1000)
#f = interp1d(x, y, kind='cubic')
#plt.plot(x_, f(x_)*2.6, label='monoclinic hafnia', linewidth=2,
#color='#ff0000', zorder=0, alpha=1.)

print 'Done with total rdf'

x1, ys = rdf_triclinic(a, 200, dist=8.)

for i, c in zip(range(3), ['#AB6A67', '#085D9E', '#C76D28']):
    plt.plot(x1, ys[i][:-1], label=ys[i][-1]+' pairs', linewidth=2, color=c)
    #f = interp1d(x1, ys[i][:-1], kind='cubic')
    #plt.plot(x_, f(x_)*2.6, label=ys[i][-1]+' pairs', linewidth=2, color=c)
...
plt.savefig('rdf_mHfO2.png', dpi=200, bbox_inches='tight')
plt.show()
