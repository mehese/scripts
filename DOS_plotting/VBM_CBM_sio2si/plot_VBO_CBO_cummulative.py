...
for c in cells_to_check:
    ...
    for i, at in enumerate(s.atoms):
        ...
        if at.species == 'Si' and limits[c]['blk'][0] < at.z < limits[c]['blk'][1]:
            ...
            y_b_u, y_b_d = y_b_u+u, y_b_d+d
        # For atoms in the bulk phase of the SiO2 layer
        elif at.species == 'O' and limits[c]['ox'][0] < at.z < limits[c]['ox'][1]:
            _, u, d = get_at_pdos(c, i+1) 
            y_o_u, y_o_d = y_o_u+u, y_o_d+d
        #else:
        #    _, u, d = get_at_pdos(c, i+1) 
        #    y_i_u, y_i_d = y_i_u+u, y_i_d+d

plt.plot(x, y_b_u, 'b-', label='Bulk Si', linewidth=2)
plt.plot(x, y_b_d, 'b-', linewidth=2)
plt.plot(x, y_o_u, 'r-', label='Bulk SiO2')
plt.plot(x, y_o_d, 'r-')
plt.plot(x, 30*y_o_u, 'r-', label='30 x Bulk SiO2')
plt.plot(x, 30*y_o_d, 'r-')
#plt.plot(x, y_i_u, 'g-', label='Interface')
#plt.plot(x, y_i_d, 'g-')
